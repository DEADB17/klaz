...
/** @typedef {string | (string | string[])[]} KpArgs*/
/** @typedef {Record<string, KpArgs>} KpArgsRecord*/
/** @typedef {Record<string, KpArgsRecord>} KpArgsRecord2*/
/** @typedef {Record<string, KpArgsRecord2>} KpArgsRecord3*/
/** @typedef {Record<string, KpArgsRecord3>} KpArgsRecord4*/

/**
 * @arg {KpArgs | KpArgsRecord4} args
 * @arg {string} [prefix]
 * @return {string}
 */
export function kzPrefix(args, prefix = "") {
  if (args == null) return "";
  if (prefix && !prefix.endsWith(":")) prefix += ":";
  if (args.constructor === Object) {
    return Object.keys(args)
      .map((it) => {
        return kzPrefix(args[it], prefix + it);
      })
      .join("");
  }
  return x(args, prefix);
}

const rxSemiSp = new RegExp("\\s*(;\\s*)+", "g");
const rxColonSp = new RegExp("\\s*(:\\s*)+", "g");

/**
 * @arg {KpArgs} args
 * @arg {string} prefix
 */
function x(args, prefix) {
        ...
        .map((arg) => {
          if (Array.isArray(arg))
            return (
              ...
              arg
                .map((it) => {
                  return it.replace(rxSemiSp, "").replace(rxColonSp, "").trim();
                })
                .join(":")
            );
          return prefix + arg.replace(rxSemiSp, "").trim();
        })
        .join(";") + ";"
    );
  }
...
export function klaz(userSpecs) {
  ...
  /** @arg {string[]} strings */
  function ks(...strings) {
    return parse(styleSheet, tab, strings.join(";"));
  }
   ...
   * @return {string}
   */
  function kp(args, prefix = "") {
    return parse(styleSheet, tab, kp(args, prefix));
  }
